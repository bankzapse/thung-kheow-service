...
export function buildFlowWide(cfg: FlowConfig): BuiltSvg {
  ...
  const numBadge = (i: number, x: number) => {
    ...
    return `<circle cx="${bx}" cy="${by}" r="60" fill="#fff"/><circle cx="${bx}" cy="${by}" r="60" fill="none" stroke="${p.circleTo}" stroke-width="6"/><text x="${bx}" y="${by + 22}" font-family="${ff("steps")}" font-size="${fs("steps", 64)}" font-weight="700" fill="${p.circleTo}" text-anchor="middle">${i + 1}</text>`;
  };
  ...
  const step = (i: number) => {
    ...
    const x = cx(i);
    const desc = s.lines
      .map((l, k) => `<text x="${x}" y="${CIRCLE_Y + R + 160 + k * 76}" font-family="${ff("steps")}" font-size="${fs("steps", 44)}" fill="${S.steps.body}" text-anchor="middle">${esc(l)}</text>`)
      .join("");
    const title = `<text x="${x}" y="${CIRCLE_Y + R + 92}" font-family="${ff("steps")}" font-size="${fs("steps", 72)}" font-weight="700" fill="${S.steps.title}" text-anchor="middle">${esc(s.title)}</text>`;
    ...
    if (i === 0) {
      ...
      const badge1 = `<circle cx="${b1x}" cy="${b1y}" r="58" fill="#fff"/><circle cx="${b1x}" cy="${b1y}" r="58" fill="none" stroke="${p.circleTo}" stroke-width="6"/><text x="${b1x}" y="${b1y + 22}" font-family="${ff("steps")}" font-size="${fs("steps", 62)}" font-weight="700" fill="${p.circleTo}" text-anchor="middle">1</text>`;
  ...
  const items = cfg.materials
    .map((m, i) => {
      ...
      return `<clipPath id="wclip${i}"><rect x="${mx - thumb / 2}" y="${ty}" width="${thumb}" height="${thumb}" rx="44"/></clipPath><image href="${m.img}" x="${mx - thumb / 2}" y="${ty}" width="${thumb}" height="${thumb}" preserveAspectRatio="xMidYMid slice" clip-path="url(#wclip${i})"/><rect x="${mx - thumb / 2}" y="${ty}" width="${thumb}" height="${thumb}" rx="44" fill="none" stroke="#e3ece6" stroke-width="4"/><text x="${mx}" y="${ty + thumb + 58}" font-family="${ff("materials")}" font-size="${fs("materials", 48)}" font-weight="600" fill="${S.materials.title}" text-anchor="middle">${esc(m.label)}</text>`;
    })
    .join("");
  ...
  const warn = `<rect x="${pillX}" y="${pillY}" width="${pillW}" height="${pillH}" rx="${pillH / 2}" fill="#dc2626"/><circle cx="${icx}" cy="${icy}" r="26" fill="#ffffff"/><path d="M${icx - 11} ${icy - 11}L${icx + 11} ${icy + 11}M${icx + 11} ${icy - 11}L${icx - 11} ${icy + 11}" stroke="#dc2626" stroke-width="7" stroke-linecap="round"/><text x="${icx + 46}" y="${icy + 16}" font-family="${ff("materials")}" font-size="${fs("materials", 44)}" font-weight="700" fill="#ffffff">${esc(cfg.warnPill)}</text>`;
  const materialStrip = `<rect x="${MARGIN}" y="${BOT_Y}" width="${SPLIT - MARGIN - 60}" height="${BOT_H}" rx="44" fill="#ffffff" stroke="#e3ece6" stroke-width="3"/><text x="${innerX}" y="${BOT_Y + 96}" font-family="${ff("materials")}" font-size="${fs("materials", 62)}" font-weight="700" fill="${S.materials.title}">${esc(cfg.materialsHeading)}</text><text x="${innerX}" y="${BOT_Y + 158}" font-family="${ff("materials")}" font-size="${fs("materials", 42)}" fill="${S.materials.body}">${esc(cfg.materialsSub)}</text>${warn}${items}`;
  ...
  const promoCard = `<rect x="${cardX}" y="${BOT_Y}" width="${cardW}" height="${qcH}" rx="44" fill="url(#gcard)"/>${gift}<text x="${tx}" y="${BOT_Y + 196}" font-family="${ff("promo")}" font-size="${fs("promo", 76)}" font-weight="700" fill="${S.promo.title}">${esc(cfg.promo.heading)}</text><text x="${tx}" y="${BOT_Y + 272}" font-family="${ff("promo")}" font-size="${fs("promo", 46)}" fill="${S.promo.body}">${esc(cfg.promo.sub)}</text><rect x="${tx}" y="${BOT_Y + 322}" width="500" height="84" rx="42" fill="#ffffff"/><text x="${tx + 250}" y="${BOT_Y + 378}" font-family="${ff("promo")}" font-size="${fs("promo", 46)}" font-weight="700" fill="${p.circleTo}" text-anchor="middle">${esc(cfg.promo.pill)}</text>`;
  ...
  const wtx = triX + 110;
  const warnCard = `<rect x="${cardX}" y="${wcY}" width="${cardW}" height="${wcH}" rx="44" fill="#fff5f5" stroke="#f2b5b5" stroke-width="3"/>${tri}<text x="${wtx}" y="${wcY + wcH / 2 - 22}" font-family="${ff("footer")}" font-size="${fs("footer", 54)}" font-weight="700" fill="${S.footer.title}">${esc(cfg.legalWarn[0] ?? "")}</text><text x="${wtx}" y="${wcY + wcH / 2 + 52}" font-family="${ff("footer")}" font-size="${fs("footer", 54)}" font-weight="700" fill="${S.footer.title}">${esc(cfg.legalWarn[1] ?? "")}</text>`;
  ...
  <text x="${MARGIN + 224}" y="152" font-family="${ff("header")}" font-size="${fs("header", 108)}" font-weight="700" fill="${S.header.title}">${esc(cfg.headerTitle)}</text>
  <text x="${MARGIN + 224}" y="226" font-family="${ff("header")}" font-size="${fs("header", 46)}" fill="${S.header.body}">${esc(cfg.headerSubtitle)}</text>
  <text x="${W - MARGIN}" y="192" font-family="${ff("header")}" font-size="${fs("header", 84)}" font-weight="700" fill="${S.header.title}" text-anchor="end">${esc(cfg.headerRight)}</text>
  ${connectors}
  ...
  <text x="${MARGIN}" y="${H - 32}" font-family="${ff("footer")}" font-size="${fs("footer", 44)}" fill="${S.footer.body}">${esc(cfg.footerLeft)}</text>
  <text x="${W / 2}" y="${H - 32}" font-family="${ff("footer")}" font-size="${fs("footer", 44)}" fill="${S.footer.body}" text-anchor="middle">${esc(cfg.footerCenter)}</text>
  <text x="${W - MARGIN}" y="${H - 32}" font-family="${ff("footer")}" font-size="${fs("footer", 44)}" font-weight="600" fill="${S.footer.body}" text-anchor="end">${esc(cfg.footerRight)}</text>
</svg>`;
